...
.PHONY: install spar ui validate qualify submit test clean check-no-key doctor
...
validate:
	@if [ -z "$(WORLD)" ]; then \
		echo "no world exported - run 'make check-world'"; \
		exit 1; \
	fi
...
validate-bots:
	@for b in rookie operator adversary; do \
		$(BIN)/python validate_deck.py bots/$$b/deck.json bots/$$b/lineup.json --world $(dir $(WORLD)); \
	done
...
submit: validate
	@if [ -z "$(TEAM)" ]; then \
		echo "usage: make submit TEAM=<your-team-name>"; \
		exit 1; \
	fi
...
check-referee:
	@if [ ! -d kit/referee ]; then \
		echo "kit/referee missing - ask your instructor to run tools.sync_referee"; \
		exit 1; \
	fi
...
check-world:
	@if [ ! -d "kit/world" ] || [ -z "$$(ls kit/world/*/manifest.json 2>/dev/null)" ]; then \
		echo "no world in kit/world/ - ask your instructor for the world artifact"; \
		exit 1; \
	fi
	...
	@if [ -f "kit/world/$$(ls -1 kit/world/*/truth.json 2>/dev/null | head -1 | cut -d/ -f3)" ]; then \
		echo "FAIL: truth.json must never ship to students"; \
		exit 1; \
	fi
